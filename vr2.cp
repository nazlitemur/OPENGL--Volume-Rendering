...
void mainRender()
{
    ...
    GLuint Texture1ID  = glGetUniformLocation(g_glslProgram, "myTextureSampler");
    // Bind our texture in Texture Unit 0
    glActiveTexture(GL_TEXTURE0);
    ...
    // 3rd attribute for normals :
    
    // 3rd attribute buffer : normals [1]
...
void loadTexture()
{
    ...
    Gx = new GLfloat[width*height];
    Gy = new GLfloat[width*height];
    Gz = new GLfloat[width*height];
    ...
    for (int i =0;i<256;i++)
    {
        for (int j=0;j<256;j++)
        {
            ...
            {
                int ind =i+j+k;
                Gx[ind]=V(i+1,j,k)-V(i-1,j,k)/2;
                Gy[ind]=V(i,j+1,k)-V(i,j-1,k)/2;
                Gz[ind]=V(i,j,k+1)-V(i,j,k-1)/2;
          // dataNormals(i,j,k) = dataVolume(i+1,j,k)-dataVolume(i-1,j,k)/2,dataVolume(i,j+1,k)-dataVolume(i,j-1,k)/2,dataVolume(i,j,k+1)-dataVolume(i,j,k-1)/2;
        
               dataNormals(i,j,k)=-(Gx[ind],Gy[ind],Gz)[ind])/Norm(Gx[ind],Gy[ind],Gz)[ind]);
    ...
    // ...
    unsigned char * dataNormals = loadBMPRaw("images/cthead_assembled.bmp", width, height);
