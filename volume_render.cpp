...
GLuint textureVolumeID, textureNormalID;
...
void mainKeyboard(unsigned char key, int x, int y)
{
	...
	else if (key == '+') theta += 0.01;
	else if (key == '-') theta -= 0.01;
	else if (key == '1') threshold += 1;
	else if (key == '2') threshold -= 1;
...
void mainRender()
{
	...
	GLuint TexNormalID = glGetUniformLocation(g_glslProgram, "myTextureSamplerNormal");
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, textureNormalID);
	glUniform1i(TexNormalID, 1);
	...
	thetaID = glGetUniformLocation(g_glslProgram, "theta");
	glUniform1f(thetaID, theta);

	thresholdID = glGetUniformLocation(g_glslProgram, "threshold");
	glUniform1i(thresholdID, threshold);
	...
	for (int slice = 0; slice < 100; slice++)
	{
		for (int row = 0; row < 256; row++)
		{
			for (int col = 0; col < 256; col++)
			{
				...
				pixel_coordinate(row, col, slice, u, v); //Final one

				int u_x1, v_x1, u_x2, v_x2; //6 of these values 
				int u_y1, v_y1, u_y2, v_y2;
				int u_z1, v_z1, u_z2, v_z2;

				pixel_coordinate(row + 1, col, slice, u_x1, v_x1);
				pixel_coordinate(row - 1, col, slice, u_x2, v_x2);
				pixel_coordinate(row, col + 1, slice, u_y1, v_y1);
				pixel_coordinate(row, col - 1, slice, u_y2, v_y2);
				pixel_coordinate(row, col, slice + 1, u_z1, v_z1);
				pixel_coordinate(row, col, slice - 1, u_z2, v_z2);

				x1 = getTextureR(u_x1, v_x1, dataVolume, width, height) - getTextureR(u_x2, v_x2, dataVolume, width, height);
				x1 /= 2;


				y1 = getTextureR(u_y1, v_y1, dataVolume, width, height) - getTextureR(u_y2, v_y2, dataVolume, width, height);
				y1 /= 2;


				z1 = getTextureR(u_z1, v_z1, dataVolume, width, height) - getTextureR(u_z2, v_z2, dataVolume, width, height);
				z1 /= 2;

				double denom = (double) ( x1 * x1 + y1 * y1 + z1 * z1 );
				denom = sqrt(denom);
				...
				setTextureR(u, v, dataNormals, width, height,  floor(Nx * 255.0));
				setTextureG(u, v, dataNormals, width, height,  floor(Ny * 255.0));
				setTextureB(u, v, dataNormals, width, height,  floor(Nz * 255.0));
			}
		}
	}
